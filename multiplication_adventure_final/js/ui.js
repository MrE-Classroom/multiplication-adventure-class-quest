window.UI = (() => {
  ...
  function renderClassSelect(){
    ...
    card.innerHTML='<h1>Multiplication Adventure</h1><p class="muted">Step 1: choose your hero style. Step 2: choose your class.</p>';
  ...
  function leftPanel(){
    ...
    body.appendChild(figure(Game.heroImage(), 'hero-portrait image-portrait', c.icon));
    body.insertAdjacentHTML('beforeend', `<h3>${c.name} ${c.difficulty}</h3><p><b>${c.ability}</b><br><span class="muted">${c.abilityText}</span></p><h4>Gear</h4>`);
  ...
  function showTown(){
    ...
    const cards=[
      ...
      ['Quest Board','View and claim quests.',GameData.ui.badge,()=>showQuests()],
  ...
  function showQuestion(){
    ...
    const b=center(s.mode==='training'?'Training Area':s.mode==='boss'?'Boss Battle':'Adventure');
    const scene=el('div','battle-scene');
    const opponentSrc = s.mode==='boss' ? area?.bossImage : (area?.enemies?.[s.session.total % (area.enemies.length || 1)] || '');
    scene.appendChild(figure(Game.heroImage(),'combat-portrait hero-combat',c.icon));
    scene.appendChild(el('div','versus','×'));
    if(opponentSrc) scene.appendChild(figure(opponentSrc,'combat-portrait enemy-combat',''));
    b.appendChild(scene);
