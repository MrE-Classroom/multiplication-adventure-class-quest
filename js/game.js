(() => {
  ...
  function itemIcon(it){
    if(!it) return '';
    if(it.slot === 'aura') return `<div class="icon-token aura-token"><div class="${auraClass(it.id)}"></div></div>`;
    if(it.slot === 'frame') return `<div class="icon-token frame-token"><div class="${frameClass(it.id)}"></div></div>`;
  ...
  function render(){
    try{
      ...
      app.innerHTML = `<div class="game">
        ...
        <section class="layout"><aside class="panel hero-side"><div class="panel-title">Hero</div><div class="panel-body">${renderHero()}</div></aside><main class="panel center-panel"><div class="panel-title">${esc(centerTitle())}</div><div class="panel-body">${renderCenter()}</div></main><aside class="panel"><div class="panel-title">Quests + Coach</div><div class="panel-body">${renderQuests()}</div></aside></section>
  ...
  function renderHero(){ const s=stats(); return `${portraitStack('big')}<div class="hero-title">${esc(heroName())}</div><div class="hero-subtitle">${esc(currentClass().name)} ${currentClass().difficulty}</div><b>${esc(currentClass().ability)}</b><div class="muted">${esc(currentClass().abilityText)}</div><div class="divider"></div><b>Gear</b>${slots.map(slot=>{const it=itemById(state.equipped[slot]);return `<div class="gear-row"><span>${cap(slot)}</span><b>${it?esc(it.name):'None'}</b><span></span></div>`}).join('')}<div class="divider"></div><b>Stats</b><div class="gear-row"><span>Attack</span><b>${s.attack}</b><span></span></div><div class="gear-row"><span>Defense</span><b>${s.defense}</b><span></span></div><div class="gear-row"><span>Speed</span><b>${s.speed}</b><span></span></div><div class="gear-row"><span>Focus</span><b>${s.focus}</b><span></span></div>`; }
  ...
  function renderBattle(){ const s=state.session, a=areaById(s.areaId), q=s.question; return `<section class="scene-panel" style="${sceneStyle()}"><div class="battle-layout"><div class="area-label">${esc(a?.name || 'Training')} Adventure · ${a?`Focus facts: ${a.focus.join(', ')}`:'Training practice'}</div><div class="fighters"><div class="fighter-box">${portraitStack('small')}<span>${esc(heroName())}</span></div><div class="versus">×</div><div class="fighter-box">${enemyArt(s.enemy, `${a?.name || 'Training'} Enemy`)}</div></div>${questionCard(`${a?.name || 'Training'} · Question ${s.index+1}/${s.total}`)}</div></section>`; }
  function renderBoss(){ const s=state.session, a=areaById(s.areaId), hpPct=Math.max(0,Math.round((s.bossHp/s.bossMaxHp)*100)); return `<section class="scene-panel" style="${sceneStyle()}"><div class="boss-layout"><div class="area-label">${esc(a.name)} Boss Battle · ${esc(a.boss)}</div><div class="boss-hp"><div class="boss-fill" style="width:${hpPct}%"></div><div class="boss-text">${esc(a.boss)} HP ${s.bossHp}/${s.bossMaxHp}</div></div><div class="fighters"><div class="fighter-box">${portraitStack('small')}<span>${esc(heroName())}</span></div><div class="versus">VS</div><div class="fighter-box">${enemyArt(a.bossImage,a.boss,'boss-art')}</div></div>${questionCard(`${a.name} Boss · Question ${s.index+1} · Damage until HP reaches 0`)}</div></section>`; }
  ...
  function previewItem(id){ const it=itemById(id); if(!it) return; const owned=state.inventory.includes(id), locked=!isUnlocked(it), canAfford=state.coins>=it.cost; const ov={}; if(it.slot==='frame') ov.frame=it.id; if(it.slot==='aura') ov.aura=it.id; if(it.slot==='pet') ov.pet=it.id; if(it.slot==='trail') ov.trail=it.id; modal=`<div class="modal-backdrop" onclick="Game.closeModal(event)"><div class="modal" onclick="event.stopPropagation()"><h2>Preview Item</h2><div class="preview-layout"><div>${['frame','aura','pet','trail'].includes(it.slot)?portraitStack('big',ov):itemIcon(it)}</div><div><h3>${esc(it.name)}</h3><p>${esc(it.rarity)} · ${it.cost} coins</p><p class="muted">${esc(it.desc)}</p><p>${locked?'Locked: '+esc(D.unlockLabels[it.unlock]||it.unlock):(owned?'Owned':'Available')}</p></div></div><div class="modal-actions"><button onclick="Game.closeModal()">Close</button>${owned?`<button class="primary" onclick="Game.equip('${it.id}')">Equip</button>`:`<button class="primary" ${locked||!canAfford?'disabled':''} onclick="Game.buy('${it.id}')">Buy</button>`}</div></div></div>`; render(); }
  ...
  function renderSelect(){ const active=D.classes[selectedClass]; app.innerHTML=`<div class="hero-select"><div class="select-card"><h1>Multiplication Adventure</h1><p class="muted">Choose your hero style and class.</p><div class="start-row"><button class="primary start-game-btn" onclick="Game.startNew('${selectedClass}','${selectedGender}')">Start as ${esc(active.heroNames[selectedGender])}</button></div><div class="avatar-picks">${['boy','girl'].map(g=>`<button class="choice-btn ${selectedGender===g?'active':''}" onclick="Game.pickGender('${g}')">${portraitStackSelect(selectedClass,g)}<b>${cap(g)} Hero</b></button>`).join('')}</div><div class="class-choices">${Object.values(D.classes).map(k=>`<div class="class-choice ${selectedClass===k.id?'active':''}" onclick="Game.pickClass('${k.id}')">${portraitStackSelect(k.id,selectedGender)}<h2>${esc(k.name)}</h2><div>${k.difficulty}</div><p><b>${esc(k.ability)}:</b> ${esc(k.abilityText)}</p><p class="muted">${esc(k.heroNames[selectedGender])} · HP ${k.hp} · Mana ${k.mana}</p><button class="primary mini-start" onclick="event.stopPropagation();Game.startNew('${k.id}','${selectedGender}')">Choose ${esc(k.heroNames[selectedGender])}</button></div>`).join('')}</div></div></div>`; }
  ...
  window.Game = {
    ...
    _state:()=>state,_render:render,_fresh:freshState,_shopHTML:renderShop
  };
  migrate();
  render();
})();
